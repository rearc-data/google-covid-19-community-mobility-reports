...
(cd pre-processing/pre-processing-code && rm -r ./pre-processing-code.zip || true; pip install numpy pytz --platform manylinux1_x86_64 --no-deps --only-binary=:all: --python-version 37 --abi cp37m --target . && zip -r pre-processing-code.zip . -x "*.dist-info/*" -x "bin/*" -x "**/__pycache__/*" && rm -r ./*/)
